...
export default function Reflection() {
  ...
  const fetchQuestions = useCallback(async () => {
    try {
      ...
      // Get active questions for the category
      ...
      // If current question index is beyond new questions length, reset to 0
      if (formattedQuestions.length > 0 && currentQuestionIndex >= formattedQuestions.length) {
  ...
  useEffect(() => {
    ...
    const setupRealtimeSubscriptions = async () => {
      ...
      // Set up real-time subscription for quiz_questions
      questionChannel = supabase
        ...
        .on(
          ...
          (payload) => {
            console.log('Question changed (realtime):', payload.eventType, payload);
            fetchQuestions();
          }
        )
        .subscribe((status) => {
      ...
      // Set up real-time subscription for quiz_categories
      categoryChannel = supabase
        ...
        .on(
          ...
          (payload) => {
            console.log('Category changed (realtime):', payload.eventType);
            fetchQuestions();
          }
        )
        .subscribe((status) => {
            ...
            <View style={styles.resultsModal}>
              ...
              <View style={styles.resultsBody}>
                ...
                <View style={styles.resultsFeedback}>
                  ...
                  {score.correct >= score.total * 0.7 && score.correct < score.total && (
                    <Text style={styles.feedbackText}>
                      👏 Great job! You're doing well!
                    </Text>
                  )}
                  {score.correct >= score.total * 0.5 && score.correct < score.total * 0.7 && (
                    <Text style={styles.feedbackText}>
